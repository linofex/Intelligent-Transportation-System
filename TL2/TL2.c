...
static bool schedule(){
	if(cross_queue_SECOND.queued  && !cross_queue_MAIN.queued)  return true; // MAIN is empty
	if(!cross_queue_SECOND.queued && cross_queue_MAIN.queued)  return false; // MAIN is not empty
	// Both streets req
	if(cross_queue_MAIN.vehicle == EMERG && cross_queue_SECOND.vehicle == NORM ) return false; // Second has higher priority
	if(cross_queue_SECOND.vehicle==EMERG && cross_queue_MAIN.vehicle == NORM ) return true;
	return false; // MAIN
}
...
static void broadcast_recv(struct broadcast_conn *c, const linkaddr_t *from){
	...
	if (request.road == MAIN){
		...
		printf("\n[TL2] Request queued on the SECOND street with vehicle: %s\n", (request.vehicle == 0)? "NORMAL" : "EMERGENCY");
...
PROCESS_THREAD(sem_TL2, ev, data){
	...
	while(1){
		...
		if (DEBUG){
			printf("[TL2] Battery level: %d", battery_level);
		}
	}
